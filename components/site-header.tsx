  ...
  return (
    <header className="sticky top-0 z-30 border-b border-black/10 bg-[color:var(--color-bg)]/95 backdrop-blur">
      <div className="mx-auto flex min-h-16 w-full max-w-6xl items-center justify-between gap-4 px-5 md:min-h-[72px] md:px-8">
        <Link href="/" className="inline-flex items-center font-display text-2xl tracking-wide text-[color:var(--color-primary)]">
          🥩 cotalos.be
        </Link>
        <nav aria-label="Navigation principale" className="hidden items-center gap-6 text-sm md:flex">
          {headerLinks.map((link) => (
            <Link key={link.href} href={link.href} className="transition-colors hover:text-[color:var(--color-primary)]">
              {link.label}
            </Link>
          ))}
        </nav>
        ...
        <div className="hidden md:block">
          <Link
            href="/#commercants"
            className="inline-flex h-10 items-center rounded-full bg-[color:var(--color-primary)] px-5 text-sm font-semibold text-[color:var(--color-bg)] hover:bg-[color:var(--color-primary)]/92"
            data-track="header_merchant_cta"
          >
          ...
          <details className="group relative">
            ...
            <div className="absolute right-0 mt-2 w-56 rounded-xl border border-black/10 bg-white p-2 shadow-premium">
              <nav aria-label="Navigation mobile" className="flex flex-col gap-1">
                {headerLinks.map((link) => (
                  <Link key={link.href} href={link.href} className="rounded-md px-3 py-2 text-sm hover:bg-[color:var(--color-primary)]/8">
                    {link.label}
                  </Link>
                ))}
                <Link href="/mentions-legales" className="rounded-md px-3 py-2 text-sm hover:bg-[color:var(--color-primary)]/8">
                  Mentions legales
                </Link>
              </nav>
            </div>
