  ...
  return (
    <div className="relative isolate overflow-hidden">
      ...
      <div className="mx-auto w-full max-w-6xl space-y-14 px-6 pb-6 pt-10 md:space-y-20 md:px-8 md:pb-12 md:pt-14">
          ...
          <div className="relative grid gap-8 lg:items-start">
            <div className="space-y-7">
              ...
              <p className="max-w-2xl text-base leading-7 text-black/75 md:text-lg">
                Trouvez votre boucherie ou votre traiteur par ville, code postal ou son nom
              </p>
        ...
        <section id="comment-ca-marche" className="reveal space-y-6" style={{ animationDelay: "140ms" }}>
          <div className="flex flex-wrap items-end justify-between gap-4">
            <h2 className="font-display text-3xl text-[color:var(--color-primary)] md:text-4xl">Comment ca marche</h2>
          </div>
          <div className="grid gap-4 md:grid-cols-3">
            {steps.map((step) => (
              ...
              >
                ...
                <h3 className="font-display text-2xl text-[color:var(--color-primary)]">{step.title}</h3>
                <p className="mt-3 text-sm leading-6 text-black/75 md:text-base">{step.description}</p>
              </article>
            ))}
          </div>
        </section>
        ...
        >
          ...
          <div className="relative grid gap-6 md:grid-cols-[1.2fr_0.8fr] md:items-center">
            <div className="space-y-4">
              <p className="text-xs uppercase tracking-[0.2em] text-[color:var(--color-accent)]">Espace commercants</p>
              <h2 className="font-display text-3xl md:text-4xl">Vous etes boucher ou traiteur ?</h2>
              <p className="max-w-2xl text-sm leading-7 text-[color:var(--color-bg)]/85 md:text-base">
                Activez la precommande en ligne, reduisez les appels repetitifs et simplifiez vos retraits en boutique.
              </p>
            </div>
        ...
        <section id="faq" className="reveal space-y-5 pb-4" style={{ animationDelay: "260ms" }}>
          <h2 className="font-display text-3xl text-[color:var(--color-primary)] md:text-4xl">FAQ</h2>
          <div className="grid gap-1 md:grid-cols-1 md:gap-x-6">
            {faqItems.map((item) => (
              <details key={item.question} className="border-b border-black/12 py-4 md:col-span-1">
